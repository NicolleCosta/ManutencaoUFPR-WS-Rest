...
<html lang="pt-BR">
    ...
    <div class="table-secondary">
        <table class="table align-middle mb-0 bg-white table-hover">
            ...
            <tbody data-bs-toggle="modal" data-bs-target="#modalOdermDeServico">
                <c:forEach var="ordem" items="${requestScope.ordensServico}">
                    <tr>
                        <td>
                        ...
                        <td>
                            <p class="fw-normal mb-1">
                                <fmt:formatDate value="${ordem.dataAbertura}" pattern="dd/MM/yyyy"/>
                            </p>
                        </td>
                        <td>
                            <p class="fw-normal mb-1">
                        ...
                        <td>
                            <span class="badge badge-sm c-status" style="background-color:
                                  <c:choose>
                                      <c:when test="${ordem.dataFinalizacao eq null}"> orange
                                          <c:set var="status" value="Aberta" />
                                      </c:when>
                                      <c:otherwise>green
                                          <c:set var="status" value="Finalizada" />
                                      </c:otherwise>
                                  </c:choose>;">
                                <c:out value="${status}"/>
                            </span>
                        ...
                        <td>
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalOdermDeServico<c:out value="${ordem.id}" />" data-id="${ordem.id}">
                                Detalhes
                            </button>
                        </td>
                    </tr>
    ...
    <c:forEach var="ordem" items="${requestScope.ordensServico}">
        <div class="modal fade" id="modalOdermDeServico<c:out value="${ordem.id}" />"  data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="chamadoModalLabel" aria-hidden="true">
